...
async def image_generations(request: Request, user_info: dict = Depends(verify_token)):
                ...
                try:
                    ...
                    if requested_model == "seedream5.0":
                        if "data" in resp_json and isinstance(resp_json["data"], list) and len(resp_json["data"]) > 0:
                            ...
                            logger.warning(f"[Seedream] 响应缺少 data 数组，keys: {list(resp_json.keys())}")
                            return _generic_error("Seedream 响应格式异常", 500)
                    ...
                    logger.info(f"[ImageGen] find_image 结果: {final_image[:80] if final_image else 'None'}")
                    if final_image:
                        if final_image.startswith("http"):
                            # 🌐 远程 CDN URL：先尝试同步下载保存（30s 超时），避免返给前端外链导致跨域显示异常
                            logger.info(f"[ImageGen] CDN 链接，尝试同步持久化 (30s 超时): {final_image[:80]}")
                            try:
                                permanent_url = await asyncio.wait_for(
                                    save_media_permanently(final_image, "png", client),
                                    timeout=30.0
                                )
                                if permanent_url.startswith("/v1/static/media/"):
                                    logger.info(f"[ImageGen] 持久化成功: {permanent_url}")
                                    return JSONResponse(status_code=200, content={"data": [{"url": permanent_url}]})
                                else:
                                    # save_media_permanently 返回了原始 URL（下载失败降级）
                                    logger.warning(f"[ImageGen] 持久化降级，返回 CDN 链接")
                                    return JSONResponse(status_code=200, content={"data": [{"url": final_image}]})
                            except asyncio.TimeoutError:
                                logger.warning(f"[ImageGen] CDN 下载超时 (30s)，返回 CDN 链接 + 后台保存")
                                asyncio.create_task(save_media_permanently(final_image, "png", client))
                                return JSONResponse(status_code=200, content={"data": [{"url": final_image}]})
                            except Exception as e:
                                logger.warning(f"[ImageGen] CDN 下载异常: {e}，返回 CDN 链接")
                                return JSONResponse(status_code=200, content={"data": [{"url": final_image}]})
                        else:
                            # 📦 base64 / 本地文件：需先持久化再返回
                            logger.info(f"[ImageGen] 开始持久化存储: {final_image[:80]}")
                            permanent_url = await save_media_permanently(final_image, "png", client)
                            logger.info(f"[ImageGen] 持久化完成: {permanent_url}")
                            return JSONResponse(status_code=200, content={"data": [{"url": permanent_url}]})
                    else:
                        logger.warning(f"[ImageGen] find_image 未找到图片！上游响应 keys: {list(resp_json.keys()) if isinstance(resp_json, dict) else type(resp_json)}")
                except Exception:
                    logger.error(f"[ImageGen] 响应解析或保存失败: {traceback.format_exc()}")
                    pass
                break 
            else:
    ...
    if last_response is not None:
        ...
        logger.warning(f"[ImageGen] 解析未命中，兜底返回上游原始响应 (status={last_response.status_code})")
